# icoar_agent.py — robust visualize + text-analysis intent + remembers last raw/clean/analyzed files
# FULL UPDATED FILE (drop-in)
#
# Fixes included (your current bug):
# 1) Explicit action parsing happens BEFORE fuzzy intent checks
# 2) action parsing uses RAW user_input (preserves /Users/... case)
# 3) action regex is IGNORECASE and does NOT lowercase file paths
#
# Keeps your features:
# - collect/clean/visualize/analyze pipeline
# - returns analyze actions for ALL tasks (toxicity/sentiment/hate/cyberbullying/emotion)
# - "text analysis" intent auto-detects task + uses last clean/raw
# - keyword extraction avoids turning "past 10 days" into keywords
...
_ABUSE_SYNONYMS = {
    ...
    "hate": ["hate", "hateful", "online hate", "hate speech", "hateful content"],
    "cyberbullying": ["cyberbullying", "cyber bullying", "bullying", "cyber-abuse", "cyber abuse"],
    "toxicity": ["toxic", "toxicity", "toxic content", "abusive language"],
...
_EXTRA_STOP = {
    ...
    "posts", "post", "data", "results", "items", "entries",
    "recent", "today", "yesterday", "last", "week", "month", "year", "days", "day", "past",
    "this", "that", "these", "those",
    "ago", "within", "during", "from", "until", "till",
}

_ANALYSIS_TASKS = ["toxicity", "sentiment", "hate", "cyberbullying", "emotion"]

_TASK_LABELS = {
    "toxicity": "Text Analysis: Toxicity",
    "sentiment": "Text Analysis: Sentiment",
    "hate": "Text Analysis: Hate",
    "cyberbullying": "Text Analysis: Cyberbullying",
    "emotion": "Text Analysis: Emotion",
}
...
def _extract_time_hint(user_text: str) -> Dict[str, Any]:
    ...
    m = re.search(r"\b(?:past|last)\s+(\d{1,3})\s+(day|days|week|weeks|month|months|year|years)\b", t)
    if m:
        val = _clip_count(m.group(1))
        ...
        if unit_raw.startswith("week"):
            ...
            unit = "year"
        return {"time_hint": {"unit": unit, "value": val}}
...
def _extract_keywords(user_text: str) -> List[str]:
    ...
    if quoted:
        ...
        for q in quoted:
            q = q.strip()
            if q and not q.isdigit() and q.lower() not in _EXTRA_STOP:
                out.append(q)
        return out[:5]
...
def _canonize_keywords(kw) -> List[str]:
    ...
    for w in parts:
        wl = w.lower()
        if wl in _EXTRA_STOP or wl.isdigit():
            continue
        matched = None
        for canonical, variants in _ABUSE_SYNONYMS.items():
...
def _build_query_from_text(user_text: str, platform: Optional[str]) -> Dict[str, Any]:
    ...
    keywords = abuse_keys or _extract_keywords(user_text)
...
def _safe_args_from_user_text(user_input: str) -> Dict[str, Any]:
    ...
    method = "Scraper" if plat in ("reddit", "facebook") else "API"
    return {"platform": plat, "method": method, "query": qry}
...
TOOLS = [
    ...
    },
    {
        "type": "function",
        "function": {
            "name": "run_text_analysis_tool",
            "description": "Run a text analysis task (toxicity/sentiment/hate/cyberbullying/emotion) on a CSV file and save a new CSV.",
            "parameters": {
                "type": "object",
                "properties": {
                    "file_path": {"type": "string", "description": "Path to input CSV file"},
                    "task": {"type": "string", "description": "One of: toxicity, sentiment, hate, cyberbullying, emotion"},
                },
...
def _ensure_assistant() -> str:
    ...
    a = client.beta.assistants.create(
        ...
        instructions=(
            ...
            "Be brief.\n"
            "When collecting data, call run_collect_tool with structured arguments.\n"
            "When the user asks to analyze a CSV (toxicity/sentiment/hate/cyberbullying/emotion), call run_text_analysis_tool.\n"
...
def _handle_tool_calls(thread_id: str, run_id: str, tool_calls) -> None:
    ...
    for call in tool_calls:
        ...
        elif name == "run_text_analysis_tool":
            result = run_text_analysis_tool(args)
        else:
    ...
    try:
        ...
        if platform == "facebook":
            ...
            if not query.get("facebook.email") or not query.get("facebook.password"):
                return {"status": "error", "message": "Facebook credentials are missing."}

        # remove unsupported keys for runner
...
def _fix_path_case(p: str) -> str:
    # Keep this for your project folder naming if you need it; do NOT lowercase /Users paths.
    return os.path.abspath(p).replace("/icoar/", "/ICOAR/")


def run_text_analysis_tool(inputs: Dict[str, Any]) -> Dict[str, Any]:
    from tabs.validation.validation import run_text_analysis_file 
    
    try:
        file_path = (inputs.get("file_path") or "").strip()
        task = (inputs.get("task") or "toxicity").strip().lower()

        if not file_path:
            return {"status": "error", "message": "Missing file_path."}

        file_path = _fix_path_case(file_path)

        if file_path.lower() in ("last", "latest", "recent"):
            file_path = _get_last_file("clean", "raw") or ""

        if not file_path or not os.path.exists(file_path):
            return {"status": "error", "message": f"File not found: {file_path}"}

        if task not in _ANALYSIS_TASKS:
            task = "toxicity"

        out_path, rows, added_cols = run_text_analysis_file(file_path, task=task)

        _set_last_file("analyzed", out_path)
        ...
        return {
            ...
            "message": f"Saved {rows} rows to {out_path} (added columns: {added_cols})",
        }
...
# =========================================================
# 6) ACTIONS HELPERS (for UI)
# =========================================================
def build_actions_for_file(fpath: Optional[str], include_clean: bool = True) -> List[Dict[str, Any]]:
    if not fpath:
        return []

    actions: List[Dict[str, Any]] = []
    if include_clean:
        actions.append({"type": "clean", "label": "Clean Text", "file": fpath})

    actions.append({"type": "visualize", "label": "Visualize", "file": fpath})

    for t in _ANALYSIS_TASKS:
        actions.append({"type": "analyze", "label": _TASK_LABELS[t], "file": fpath, "task": t})

    actions.append({"type": "export", "label": "Export to Excel", "file": fpath})
    return actions


def split_actions(actions: List[Dict[str, Any]]) -> Dict[str, List[Dict[str, Any]]]:
    basic, analysis = [], []
    for a in actions or []:
        if a.get("type") == "analyze":
            analysis.append(a)
        else:
            basic.append(a)
...
def run_clean_step(file_path: str) -> Dict[str, Any]:
    ...
    if abs_input.endswith("__clean.csv"):
        return {
            "text": f"### Clean / Preprocess\n\n`{abs_input}` is already a cleaned file. You can visualize or run text analysis.",
            "file": abs_input,
            "actions": build_actions_for_file(abs_input, include_clean=False),
            "plot_png": None,
        }
    ...
    try:
        ...
        _set_last_file("clean", cleaned_path)
        ...
        return {
            "text": msg,
            "file": cleaned_path,
            "actions": build_actions_for_file(cleaned_path, include_clean=False),
            "plot_png": None,
        }
    except Exception as e:
        return {
            ...
            "file": abs_input,
            "actions": build_actions_for_file(abs_input),
            "plot_png": None,
        }
...
def run_visualize_step(file_path: str) -> Dict[str, Any]:
    ...
    if not common:
        ...
        return {"text": msg, "file": abs_input, "actions": build_actions_for_file(abs_input), "plot_png": None}
    ...
    return {"text": msg, "file": abs_input, "actions": build_actions_for_file(abs_input), "plot_png": plot_png}
...
def compose_after_collection(platform: str, query: Dict[str, Any], file_path: str, rows: Optional[int]) -> str:
    ...
    parts.append("Next steps: clean, visualize, run text analysis, or export.")
    parts.append("Choose what you want to do next below.")
    return "\n\n".join(parts)
...
def _detect_analysis_task(text: str) -> Optional[str]:
    t = (text or "").lower()

    for task in _ANALYSIS_TASKS:
        if re.search(rf"\b{re.escape(task)}\b", t):
            return task

    if re.search(r"\bhate\s*speech\b", t) or re.search(r"\bhateful\b", t):
        return "hate"
    if re.search(r"\bcyber\s*bully\w*\b", t):
        return "cyberbullying"

    toks = re.findall(r"[a-zA-Z]+", t)
    for tok in toks:
        m = _fuzzy_pick(tok, _ANALYSIS_TASKS, cutoff=0.83)
        if m:
            return m
    return None
...
def _has_analysis_intent(text: str) -> bool:
    t = (text or "").lower()
    if re.search(r"\b(text\s*analysis|analy(?:ze|se)|analysis|score|classify)\b", t):
        return True
    if _detect_analysis_task(t):
        return True
    return False
...
def run_agent_response(user_input: str) -> Dict[str, Any]:
    ...
    # =====================================================
    # 0) EXPLICIT ACTION SHORTCUT (MUST BE FIRST)
    #    Parse from raw_input to preserve /Users/... case
    # =====================================================
    m_action = re.search(
        r"action:(clean|visualize|summarize|export|analyze)\s+file:([^\s]+)(?:\s+task:(toxicity|sentiment|hate|cyberbullying|emotion))?",
        raw_input,
        flags=re.IGNORECASE,
    )

    if m_action:
        act = m_action.group(1).lower()
        fpath = m_action.group(2).strip()
        task = (m_action.group(3) or "toxicity").strip().lower()

        if fpath.lower() in ("last", "latest", "recent"):
            fpath = _get_last_file("analyzed", "clean", "raw") or fpath
        ...
        if act == "export":
            ...
            return {"text": msg, "file": _fix_path_case(fpath), "actions": build_actions_for_file(fpath), "plot_png": None}

        if act == "analyze":
            result = run_text_analysis_tool({"file_path": fpath, "task": task})
            if result.get("status") == "ok":
                outp = result["output"]
                msg = (
                    "### Text Analysis\n\n"
                    f"Task: **{task}**\n\n"
                    f"Saved output to `{outp}`"
                )
                return {"text": msg, "file": outp, "actions": build_actions_for_file(outp, include_clean=True), "plot_png": None}

            return {
                "text": f"### Text Analysis\n\nAnalysis failed: {result.get('message')}",
                "file": fpath,
                "actions": build_actions_for_file(fpath),
                "plot_png": None,
            }

    # ---------- EARLY: VISUALIZE FUZZY INTENT ----------
    if _has_visualize_intent(lowered):
        target = _get_last_file("analyzed", "clean", "raw")
        if not target:
            try:
    ...
    # ---------- EARLY: ANALYZE NATURAL INTENT ----------
    if _has_analysis_intent(lowered):
        task = _detect_analysis_task(lowered) or "toxicity"
        ...
        if target:
            res = run_text_analysis_tool({"file_path": target, "task": task})
            ...
            return {"text": f"Analysis failed: {res.get('message')}", "file": target, "actions": build_actions_for_file(target), "plot_png": None}

    # ---------- HEURISTICS (visualize -> clean -> summarize) ----------
    def _guess_target_file(keyword: str) -> Optional[str]:
        if keyword not in lowered:
            return None
        guessed = re.findall(r"(/?[\w\/\.-]+\.csv)", raw_input)
        if guessed:
            target = guessed[0]
            ...
            return target
        try:
            return (st.session_state.get("assistant_result") or {}).get("file")
        except Exception:
            return None
    ...
    try:
        fallback_args = _safe_args_from_user_text(raw_input)
        ...
        if client is not None:
            ...
            client.beta.threads.messages.create(thread_id=thread.id, role="user", content=raw_input)
            ...
            while True:
                ...
                if time.time() - started > MAX_WAIT_SECS:
                    final_text = (
                        f"Timed out waiting for assistant (status: {status}). "
            ...
            # attempt to discover output file from tool steps
            try:
        ...
        if stable_file_path:
            ...
            actions = build_actions_for_file(stable_file_path, include_clean=True)
