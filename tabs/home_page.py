...
def home_content():
    ...
    with col2:
        ...
        st.markdown("""In this tab you can analyse multi-media data. Various multi-media classification functionalities are provided in this tab.""")
        st.subheader("i. Image Classification")
        st.markdown("""A custom model has developed to categorise images to detect hateful content. The model can be used for image classification.""")
        st.subheader("ii. Meme Classification")
        st.markdown("""Classifying memes is a slightly difficult process, since changing either the caption or the text in the meme can change the context.
