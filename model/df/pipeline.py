...
class FaceFinder(Video):
    ...
    def find_coordinates(self, landmark, K=2.2):
        ...
        return ((floor(C[1]), floor(C[0])), floor(l_12), rot)
    ...
    def find_faces(
        ...
        for i in finder_frameset:
            ...
            potential_face_patch = frame[
                potential_location[0] : potential_location[2], potential_location[3] : potential_location[1]
            ]
    ...
    def get_aligned_face(self, i, l_factor=1.3):
        ...
        if i in self.coordinates:
            c, l_temp, r = self.coordinates[i]
            l_temp1 = int(l_temp) * l_factor  # fine-tuning the face zoom we really want
            dl_ = floor(np.sqrt(2) * l_temp1 / 2)  # largest zone even when rotated
            patch = self.get_image_slice(
                frame, floor(c[0] - dl_), floor(c[0] + dl_), floor(c[1] - dl_), floor(c[1] + dl_)
            )
...
class FaceBatchGenerator:
    ...
    def next_batch(self, batch_size=50):
        ...
        min(self.head + batch_size, self.length)
        i = 0
...
def compute_accuracy(classifier, dirname, frame_subsample_count=30):
    ...
    filenames = [
        ...
        if isfile(join(dirname, f)) and ((f[-4:] == ".mp4") or (f[-4:] == ".avi") or (f[-4:] == ".mov"))
    ]
    predictions = {}
